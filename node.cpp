...
// 当网络中选出leader后，leader会每隔一段时间向所有的follower发送心跳包(append_entries)
// 每当follower收到这个心跳包时就将定时器重置，一旦某个follower在定时器溢出时都没有收到
// 心跳包，就会认为已经和leader断开连接，然后变成condidate，开始新一轮的选举
void NodeImpl::handle_election_timeout() {
    ...
    // 没有超时
    if ((butil::monotonic_time_ms() - _last_leader_timestamp)  < _options.election_timeout_ms) {
        return;
    }
    ...
    // 重置leader id为空，此时当前节点会认为网络中没有leader
    reset_leader_id(empty_id, status);

    // 当定时器溢出后，follower会变成condidate，然后向网络中广播request_vote_rpc来获取投票
    // braft在这之前增加了pre_vote机制，先与集群中的节点通信，获取leader是否存活的信息
    // 如果当前集群中仍然有leader，则不会变成condidate，也不会增加任期号
    return pre_vote(&lck);
...
// 当condidate在定时器溢出后也没有收到多数节点的投票，就重新开始选举
...
void NodeImpl::handle_request_vote_response(const PeerId& peer_id, const int64_t term,
    ...
    // check response term
    // 如果响应中的任期号大于本地任期号，说明本地节点已经过期，更新任期号后变成follower返回
    ...
    if (response.granted()) {
        ...
        // 判断是否已经获得了大部分节点的投票，如果是，则变成leader
...
void NodeImpl::elect_self(std::unique_lock<raft_mutex_t>* lck) {
    ...
    // 如果之前状态是follower，则取消follower的选举定时器
    if (_state == STATE_FOLLOWER) {
        BRAFT_VLOG << "node " << _group_id << ":" << _server_id
            << " term " << _current_term << " stop election_timer";
        // 选举定时器和leader发送的心跳包对应
    ...
    // 成为condidate后，会认为集群中没有leader，所以设置leader id为空
    reset_leader_id(empty_id, status);

    // 状态变为condidate，开始新一轮的选举
    ...
    // 打开投票定时器，如果这个定时器溢出后也没有收到多数节点的投票，就重新开始选举
    _vote_timer.start();
...
void NodeImpl::step_down(const int64_t term, bool wakeup_a_candidate, 
    ...
    if (_state == STATE_CANDIDATE) {
        // follower不需要投票定时器(只有condidate在选举的过程中需要)
        _vote_timer.stop();
...
// in lock
// 重置leader id(此时状态为follower)
// 1. 当step_down时leader_id为空，表示当前还没有选举出leader
// 2. 当选举出leader时leader_id为选出的leader的id
void NodeImpl::reset_leader_id(const PeerId& new_leader_id,  const butil::Status& status) {
...
// in lock
// 收到一个request后，判断是否需要step down
void NodeImpl::check_step_down(const int64_t request_term, const PeerId& server_id) {
    butil::Status status;
    // 收到的任期号大，说明已经有新的condidate称为leader(或者正在选举)，step down为follwoer
...
// in lock
// 当一个condidate收到多数的投票后，就会调用become_leader转变为leader
void NodeImpl::become_leader() {
    ...
    // 选举时会设有定时器，如果溢出则会开始新一轮的选举，在称为leader后就可关掉了
    _vote_timer.stop();
    ...
    // 设置leader定时器(每个leader也都有一个定时器用来表示自己的任期时长)
    // 超时后调用handle_stepdown_timeout，变为follower/condidate
...
int NodeImpl::handle_request_vote_request(const RequestVoteRequest* request,
    ...
    do {
        // check term
        // 请求的任期号需要不小于当前节点保存的任期号，这样才不会被认为是过时的请求
        if (request->term() >= _current_term) {
            LOG(INFO) << "node " << _group_id << ":" << _server_id
                << " received RequestVote from " << request->server_id()
                << " in term " << request->term()
            ...
            // 请求中的任期号大于当前任期号，说明有其他节点在本地节点定时器溢出前开始称为condidate
            // 本地节点变为follower并且为这个节点投票
            if (request->term() > _current_term) {
                butil::Status status;
            ...
            LOG(INFO) << "node " << _group_id << ":" << _server_id
                ...
                << " in term " << request->term() << " current_term " << _current_term;
            break;
        }
        ...
        bool log_is_ok = (LogId(request->last_log_index(), request->last_log_term())
                          >= last_log_id);
        // save
        // 如果接收到的是一个新日志，并且当前节点没有为其他节点投票，就为它投票，同时变为follower
    ...
    // 构建响应，包括
    // 1. term: 当前任期号，用于candidate区更新自己的任期号(通常是因为本次request是一个过时的candiate发送的)
    // 2. vote_granted: bool类型，如果condidate获得这个投票则为true
    response->set_term(_current_term);
...
// follower收到leader的append_entries消息，将日志保存到本地
